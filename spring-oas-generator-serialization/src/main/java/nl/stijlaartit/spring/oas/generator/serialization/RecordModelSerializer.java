...
public class RecordModelSerializer implements GenerationFileSerializer<RecordModel> {
    ...
    JavaFile toJavaFile(RecordModel model) {
        ...
        for (RecordField field : model.fields()) {
    ...
    private MethodSpec builderSetter(ClassName builderType, RecordField field) {
        var typeName = resolveFieldType(field);
        ParameterSpec.Builder parameter = ParameterSpec.builder(typeName, field.name().value());
        if (!field.required() || field.nullable()) {
            parameter.addAnnotation(AnnotationSpec.builder(NULLABLE).build());
        }
        MethodSpec.Builder builder = MethodSpec.methodBuilder(field.name().value())
                .addModifiers(Modifier.PUBLIC)
                .returns(builderType)
                .addParameter(parameter.build());
        return builder
                .addStatement("this.$N = $N", field.name().value(), field.name().value())
                .addStatement("return this")
                .build();
    }

    private MethodSpec builderNullWrapperValueSetter(ClassName builderType, RecordField field) {
        var wrappedType = TypeNameResolver.resolve(field.type());
        ParameterSpec.Builder parameter = ParameterSpec.builder(wrappedType, field.name().value())
                .addAnnotation(AnnotationSpec.builder(NULLABLE).build());
        return MethodSpec.methodBuilder(field.name().value())
                .addModifiers(Modifier.PUBLIC)
                .returns(builderType)
                .addParameter(parameter.build())
                .addStatement("this.$N = new $T<>($N)", field.name().value(),
                        ClassName.get(modelsPackage, NULL_WRAPPER_NAME), field.name().value())
                .addStatement("return this")
                .build();
    }
    ...
    private MethodSpec builderBuildMethod(RecordModel model) {
        ...
        for (int i = 0; i < model.fields().size(); i++) {
            ...
            if (requireNonNull) {
                args.add("$T.requireNonNull($N, $S)", Objects.class, field.name().value(), field.name().value());
            } else {
                args.add("$N", field.name().value());
            }
        }
        if (model.additionalProperties()) {
    ...
    private com.palantir.javapoet.TypeName resolveFieldType(RecordField field) {
    ...
    private boolean isNullWrapperField(RecordField field) {
